...
{
    typedef std::function<std::shared_ptr<Stream>(const std::string h264_samples, const unsigned fps, const std::string opus_samples)> create_stream_callback;
...
public:
    ...
    /** 用于延时诊断（frameMark 等）向浏览器发送 JSON */
...
private:
    ...
    const std::string defaultRootDirectory = "D:\\code\\sdk\\av\\libdatachannel-0.22.6\\examples\\streamer\\samples\\";
