...
ClientPeerConnection::ClientPeerConnection(std::shared_ptr<rtc::PeerConnection> pc, std::weak_ptr<rtc::WebSocket> ws, std::string id,
    ...
    , m_file_transfer_controller(std::move(file_transfer_controller))
...
{
    input_controller::instance()->unbind_mouse_target();
}
...
void ClientPeerConnection::_on_data_channel_message(const std::string& clientId, const std::shared_ptr<rtc::DataChannel>& dc, const std::string& msg)
{
    try {
        ...
        if (m_file_transfer_controller && m_file_transfer_controller->can_handle_type(type)) {
            ...
            m_thread_queue.dispatch([this, clientId, payload = std::move(payload), dc]() mutable {
                if (m_file_transfer_controller) {
                    m_file_transfer_controller->handle_message(clientId, payload, dc);
                }
            });
            return;
        }
...
{
    // 接收端在持续丢包/抖动后请求关键帧。
    // 将操作下沉到 ClientPeerConnection 的工作队列，避免与 media_session 并发读写。
    m_thread_queue.dispatch([this, clientId]() {
        try {
            if (m_media_session.has_value() && m_media_session.value()) {
                std::cout << "[keyframe] request_force_keyframe from client=" << clientId << std::endl;
                m_media_session.value()->request_force_keyframe();
            }
        } catch (...) {}
    });
}
...
void ClientPeerConnection::set_callback(create_media_session_callback cb)
{
    m_create_media_session_callback = std::move(cb);
}
...
void ClientPeerConnection::_bind_mouse_target_for_media_session(const std::shared_ptr<remote_desktop_media_session>& media_session)
{
    if (!media_session) return;
    const HWND hwnd = media_session->get_main_window();
    if (hwnd) input_controller::instance()->bind_mouse_target(hwnd);
}

void ClientPeerConnection::_start_media_session()
{
    std::cout << "[media] _start_media_session enter client=" << m_id << std::endl;
    std::shared_ptr<remote_desktop_media_session> media_session;
    if (m_media_session.has_value()) {
        media_session = m_media_session.value();
        if (media_session && media_session->is_media_running()) {
            std::cout << "[media] already running client=" << m_id << std::endl;
            _bind_mouse_target_for_media_session(media_session);
            // 即使媒体已在运行，也要更新 sender 的客户端快照引用到当前 peer。
            media_session->attach_client_peer(std::weak_ptr<ClientPeerConnection>(shared_from_this()));
            return;
        }
    }
    else {
        if (!m_create_media_session_callback) {
            std::cout << "[media] create_media_session_callback not set client=" << m_id << std::endl;
            return;
        }
        try {
            media_session = m_create_media_session_callback();
        } catch (const std::exception& e) {
            std::cout << "[media] create_media_session_callback exception client=" << m_id << " err=" << e.what() << std::endl;
            return;
        } catch (...) {
            std::cout << "[media] create_media_session_callback unknown exception client=" << m_id << std::endl;
            return;
        }
        if (!media_session) {
            std::cout << "[media] create_media_session_callback returned null client=" << m_id << std::endl;
            return;
        }
        m_media_session = media_session;
    }
    std::cout << "[media] start for client=" << m_id << std::endl;
    _bind_mouse_target_for_media_session(media_session);
    media_session->attach_client_peer(std::weak_ptr<ClientPeerConnection>(shared_from_this()));
    media_session->start_media_session();
}
...
void ClientPeerConnection::_handle_mouse_move(const nlohmann::json& json)
{
    ...
    input_controller::instance()->simulate_mouse_move(x, y, absX, absY, videoWidth, videoHeight);
...
void ClientPeerConnection::_handle_mouse_down(const nlohmann::json& json)
{
    ...
    int x = json.value("x", 0);
    int y = json.value("y", 0);
    input_controller::instance()->simulate_mouse_down(button, x, y);
}
...
void ClientPeerConnection::_handle_mouse_up(const nlohmann::json& json)
{
	...
	int x = json.value("x", 0);
	int y = json.value("y", 0);
    input_controller::instance()->simulate_mouse_up(button, x, y);
}
...
void ClientPeerConnection::_handle_mouse_double_click(const nlohmann::json& json)
{
	...
	int x = json.value("x", 0);
	int y = json.value("y", 0);
    input_controller::instance()->simulate_mouse_double_click(button, x, y);
}
...
void ClientPeerConnection::_handle_mouse_wheel(const nlohmann::json& json)
{
	...
	int x = json.value("x", 0);
	int y = json.value("y", 0);
    input_controller::instance()->simulate_mouse_wheel(deltaX, deltaY, x, y);
}
...
void ClientPeerConnection::_handle_key_down(const nlohmann::json& json)
{
    ...
    input_controller::instance()->simulate_key_down(0, 0, vk, shiftKey, ctrlKey, altKey, metaKey);
}
...
void ClientPeerConnection::_handle_key_up(const nlohmann::json& json)
{
	...
	const int altKey = json.value("altKey", 0);
	const int metaKey = json.value("metaKey", 0);
    input_controller::instance()->simulate_key_up(0, 0, vk, shiftKey, ctrlKey, altKey, metaKey);
}
