...
bool remote_video_engine::is_remote_process_still_running() const
{
    ...
    if (m_process_ops->get_exit_code() == STILL_ACTIVE)  return true;

    if (m_process_ops->is_running(m_process_ops->capture_pid())) return true;
    if (m_process_ops->is_running(m_process_ops->launch_pid())) return true;
    return false;
}
...
void remote_video_engine::stop()
{
    ...
    try {
        if (m_process_ops) {
			std::cout << "[proc] stop: terminating processes launch_pid=" << m_process_ops->launch_pid()
                      << " capture_pid=" << m_process_ops->capture_pid() << std::endl;
            m_process_ops->terminate();
        }
    } catch (...) {
    }
}
...
void remote_video_engine::notify_remote_exit_if_needed(const char* why)
{
    ...
    try {
        std::cout << "[proc] remote exit notify why=" << (why ? why : "")
			      << " capture_pid=" << m_process_ops->capture_pid()  
...
void remote_video_engine::exit_watch_loop()
{
    while (m_running.load(std::memory_order_relaxed)) {
        if (m_process_ops) {
            ...
            const auto resolved = CaptureTargetResolver::resolve(*m_process_ops, m_main_window);
            ...
            if (owner_pid != 0) {
                ...
                if (m_process_ops->get_exit_code() != STILL_ACTIVE) {
                    const DWORD cap = m_process_ops->capture_pid();
                    const bool capturing_child = (cap != 0 && cap != m_process_ops->launch_pid() && m_process_ops->is_running(cap));
                    if (!(capturing_child)) {
...
void remote_video_engine::capture_loop()
{
    ...
    while (m_threads_running.load(std::memory_order_acquire)) {
        ...
        if (surfaces.empty()) {
            ...
            std::cout << "[capture] no surfaces found for pid=" << m_process_ops->capture_pid()
